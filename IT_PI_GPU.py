...
def calc_pi_gpu(c, basis_matrices, X):
    coef_pi = cp.dot(c, basis_matrices).reshape(1, -1)
    pi_mat = cp.ones((X.shape[0], 1))
    # iterar sobre columnas
    for i in range(coef_pi.shape[1]):
        tmp = X[:, i] ** coef_pi[:, i]
        pi_mat = cp.multiply(pi_mat, tmp.reshape(-1, 1))
    return pi_mat
...
def MI_input_output_gpu_test(
    ...
    with warnings.catch_warnings():
        ...
        try:
            pi_list = [calc_pi_gpu(cp.asarray(a, dtype=cp.float64),
                                   basis_matrices,
                                   X) for a in a_list]
            pi = cp.column_stack(pi_list)
            if optimize_output and para_o is not None:
                a_o_list = [tuple(para_o)]
                pi_o = cp.squeeze(cp.array([calc_pi(a_o, basis_matrices, X) for a_o in a_o_list]))
    ...
    if estimator == "binning":
        ...
        mi = MI_d_binning(pi.get(), np.array(Y), num_bins)
...
def main(
    ...
    def objective_fn(params):
        ...
        return MI_input_output_gpu_test(
            input_params,
            output_params,
            basis_matrices,
            X,
