...
def objective_fn_global(params, X, Y, basis_matrices, num_basis, num_input,
                        estimator, estimator_params, optimize_output):
    input_params = params[:num_basis * num_input]
    output_params = params[num_basis * num_input:] if optimize_output else None
...
def main(
    ...
    #bounds = [[-4] * num_para_total, [4] * num_para_total]
    options = {
        'bounds': bounds,
        'maxiter': maxiter,
        'tolx': 1e-8,
    ...
    with EvalParallel2(objective_partial,
                       number_of_processes=mp.cpu_count()) as eval_all:
        while not es.stop():
            solutions = es.ask()
            es.tell(solutions, eval_all(solutions))  # eval_all also accepts `fitness_function`
                                      # and `args` as optional arguments
            es.disp()
    #with mp.Pool(processes=4) as pool: # Use this if you want to limit the cores
    # with mp.Pool(mp.cpu_count()) as pool:
    #     while not es.stop():
    #         solutions = es.ask()
    #         fitness = pool.map(objective_partial, solutions)
